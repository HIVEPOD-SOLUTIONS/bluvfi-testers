...
const requiredString = (value) => typeof value === 'string' && value.trim().length > 0 ? value.trim() : '';
const getJwtSecret = () => {
...
const signToken = (user) => jsonwebtoken_1.default.sign(user, getJwtSecret(), { expiresIn: '7d' });
...
const invalidateTesterCaches = () => (0, redis_1.deleteCache)(LEADERBOARD_CACHE_KEY, STATS_CACHE_KEY);
const requireAuth = (req, res, next) => {
    ...
    try {
        req.tester = jsonwebtoken_1.default.verify(token, getJwtSecret());
        return next();
    }
    catch {
...
const buildStats = async () => {
    const cached = await (0, redis_1.readCache)(STATS_CACHE_KEY);
    if (cached)
        return cached;
    const [registered, selected, active, inactiveRemoved] = await Promise.all([
        prisma_1.prisma.testerUser.count(),
        prisma_1.prisma.testerUser.count({ where: { isSelected: true } }),
        prisma_1.prisma.testerUser.count({ where: { status: client_1.TesterStatus.ACTIVE } }),
        prisma_1.prisma.testerUser.count({
            where: {
                isSelected: false,
    ...
    await (0, redis_1.writeCache)(STATS_CACHE_KEY, stats, 30);
    return stats;
};
const buildLeaderboard = async () => {
    const cached = await (0, redis_1.readCache)(LEADERBOARD_CACHE_KEY);
    if (cached)
        return cached;
    const users = await prisma_1.prisma.testerUser.findMany({
    ...
    const rows = users
        ...
        const activeDownlineCount = user.referrals.filter((referral) => referral.status === client_1.TesterStatus.ACTIVE).length;
        const selectedDownlineCount = user.referrals.filter((referral) => referral.isSelected).length;
        return {
            id: user.id,
    ...
    await (0, redis_1.writeCache)(LEADERBOARD_CACHE_KEY, rows, 30);
    return rows;
};
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        const redisClient = await (0, redis_1.getRedis)();
        if (redisClient) {
...
router.post('/login', async (req, res) => {
    ...
    const password = requiredString(req.body.password);
    getJwtSecret();
    const user = await prisma_1.prisma.testerUser.findUnique({ where: { email } });
    if (!user) {
    ...
    await prisma_1.prisma.testerUser.update({
        where: { id: user.id },
        data: { lastActiveAt: new Date(), status: client_1.TesterStatus.ACTIVE },
    });
    await invalidateTesterCaches();
    return res.json({
        user: await prisma_1.prisma.testerUser.findUnique({ where: { id: user.id }, select: publicUserSelect }),
        token: signToken({ id: user.id, email: user.email, role: user.role }),
    });
});
router.get('/me', requireAuth, async (req, res) => {
    const user = await prisma_1.prisma.testerUser.findUnique({
...
router.post('/activity', requireAuth, async (req, res) => {
    ...
    const user = await prisma_1.prisma.testerUser.update({
        where: { id: req.tester.id },
        data: {
            status: client_1.TesterStatus.ACTIVE,
            lastActiveAt: new Date(),
            activities: {
                create: {
...
router.get('/admin/users', requireAdmin, async (_req, res) => {
    const [users, stats] = await Promise.all([
        prisma_1.prisma.testerUser.findMany({
...
router.patch('/admin/users/:id/remove-selection', requireAdmin, async (req, res) => {
    ...
    const user = await prisma_1.prisma.testerUser.update({
        ...
        data: {
            isSelected: false,
            status: client_1.TesterStatus.INACTIVE,
